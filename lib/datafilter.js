...
DataFilter.prototype.evaluatePartialExpression = function(valueSource, operator, valueFilter) {
    ...
    if(typeof operator === 'function') {
        result = !!(operator(valueSource, valueFilter));
    } else if(operatorList.hasOwnProperty(operator)) {
        result = !!(operatorList[operator](valueSource, valueFilter));
    }
...
DataFilter.prototype.evaluateExpression = function(sourceValue, operator, filterValues) {
    ...
    for(var i = 0, l = filterValues.length; i < l; i++) {
        result = this.evaluatePartialExpression(sourceValue, operator, filterValues[i]);
 ...
 */
DataFilter.addOperator = function(name, evaluationFunction) {
    var result = false;
 ...
 * @returns {boolean} Whether the operation succeed
 */
DataFilter.addOperatorAlias = function(name, alias) {
    var result = false;
...
DataFilter.addOperatorAlias('greater than equal', '>=');
DataFilter.addOperatorAlias('less than', '<');
DataFilter.addOperatorAlias('less than equal', '<=');
